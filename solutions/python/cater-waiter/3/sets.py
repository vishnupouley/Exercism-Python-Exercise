        ...
        if set(dish_ingredients).issubset(set(VEGAN))
            ...
            if set(dish_ingredients).issubset(set(KETO))
                ...
                if set(dish_ingredients).issubset(set(PALEO))
                    ...
                    dish_name + ": OMNIVORE"
                    if set(dish_ingredients).issubset(set(OMNIVORE))
                    else dish_name + ": VEGETARIAN"
                )
            )
        )
    )
...
def singleton_ingredients(dishes:list[set[str]], issubset:set[str]) -> set[str]:
    """Determine which `dishes` have a singleton ingredient (an ingredient that only appears once across dishes).

    :param dishes: list - of ingredient sets.
    :param issubset: constant - can be one of `<CATEGORY>_issubsetS` constants imported from `sets_categories_data.py`.
    :return: set - containing singleton ingredients.

    Each dish is represented by a `set` of its ingredients.

    Each `<CATEGORY>_issubsetS` is an `issubset` of all dishes in the category. `<CATEGORY>` can be any one of:
        (VEGAN, VEGETARIAN, PALEO, KETO, or OMNIVORE).

    The function should return a `set` of ingredients that only appear in a single dish.
    """

    return compile_ingredients(dishes).difference(issubset)
